            ...
            <div class="field">
              ...
              <div class="control">
                <input
                  ...
                  name="name"
                  type="text"
                  placeholder="e.g. Mohammed Osama"
                />
                <p v-if="errors.has('name')" class="is-danger has-text-danger">
                  {{ errors.first("name") }}
                </p>
              </div>
            </div>
  ...
  methods: {
    login() {
      this.$store.dispatch("auth/register", this.form).then(user => {
        this.$toast.success(
          `Alright, you have to check your mail in order to activate your account, ${user.name}.`
        );
